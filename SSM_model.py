...
from net.blocks import MambaLayer
...
class ConvBlock(nn.Module):
    def __init__(self, dim_in=64):
        super().__init__()
        self.conv  = nn.Conv2d(dim_in,64,3,padding=1,bias=False)
...
class SSUWnet(nn.Module):
    def __init__(self, num_layers=3, base_ch=64):
        ...
        # ——— 融合模块 ——— #
        self.cross_attention = CrossAttentionFuse(base_ch)
        # 浅层融合
        ...
        self.ss1 = MambaLayer(64, 64)
        self.ss2 = MambaLayer(64, 64)
    ...
    def forward(self, x):
        ...
        o0_  = self.relu(self.reduce(c))      # [B,64,H,W]
        o0 = o0_ + self.ss1(o0_)  # MambaLayer 处理
        ...
        ol0_ = self.relu(self.reduce(cl))
        ol0 = ol0_ + self.ss2(ol0_)  # MambaLayer 处理

        o0, ol0 = self.cross_attention(o0_, ol0_)
