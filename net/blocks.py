...
import torch
import torch.nn as nn
from mamba_ssm import Mamba  # 确保你有正确的 Mamba 实现（如 mamba-ssm）
...
class MambaLayer(nn.Module):
    def __init__(self, input_dim, output_dim, d_state=16, d_conv=3, expand=2):
        ...
        # 如果 input_dim ≠ output_dim，需要一个投影层
    ...
    def forward(self, x):
        ...
        # 准备输入给 Mamba: (B, C, H, W) -> (B, H*W, C)
        x_ = x.permute(0, 2, 3, 1).reshape(B, H * W, C)  # (B, H, W, C) -> (B, H*W, C)

        x1 = self.norm1(x_)
        x1 = self.mamba(x1) + self.skip_scale * x_

        x2 = self.norm2(x1)
        x2 = x2.reshape(B, H, W, C).permute(0, 3, 1, 2)  # (B, H*W, C) -> (B, H, W, C) -> (B, C, H, W)
